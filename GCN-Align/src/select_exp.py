...
def get_2_nec_kg(file, exp, split):
    ...
    # print((nec_tri1 | nec_tri2) - (tar_tri1 | tar_tri2))
...
def read_from_exp(file, num, file_out, file_path):
    d1, d2 = read_link(file_path + '/pair')
    exp = []
    with open(file) as f:
        cur_exp = []
        lines = f.readlines()
        for line in lines:
            ...
            if cur[0] == '0' and cur[1] == '0' and cur[2] == '0':
                ...
                if str(cur[0]) in d1 or str(cur[2]) in d1 or str(cur[0]) in d2 or str(cur[2]) in d2:
...
def read_from_exp_anchor(file, num, file_out, rules, file_path):
    exp = []
    d1, d2 = read_link(file_path + '/pair')
    with open(file) as f:
        cur_exp = []
    ...
    with open(file_out, 'w') as f:
        for i in range(len(exp)):
            ...
            cur_exp = []
            for tri in tmp:
                if str(tri[0]) in d1 or str(tri[2]) in d1 or str(tri[0]) in d2 or str(tri[2]) in d2:
                    cur_exp.append(tri) 
            rule = rules[i]
            need = rule[0]
            delete = rule[1]
...
def read_from_exp_lore(file, num, file_out, rules, file_path):
    exp = []
    d1, d2 = read_link(file_path + '/pair')
    with open(file) as f:
        cur_exp = []
...
if __name__ == '__main__':
    ...
    num = args.num
    method1 = 'lime'
    align_method = 'GCN-Align'
    if method == 'lore':
        if lang == 'w':
            rules = read_rule('../datasets/D_W/exp_' + method,'../datasets/D_W')
            read_from_exp_lore('../datasets/D_W/exp_' + method1, num, '../datasets/D_W/exp_'+ method +'1', rules,'../datasets/D_W')
        elif lang == 'y':
            rules = read_rule('../datasets/D_Y/exp_' + method,'../datasets/D_Y')
            read_from_exp_lore('../datasets/D_Y/exp_' + method1, num, '../datasets/D_Y/exp_'+ method +'1', rules,'../datasets/D_Y')
        else:
            rules = read_rule('../datasets/dbp_' + lang + '_e/exp_' + method,'../datasets/dbp_' + lang + '_e/')
            read_from_exp_lore('../datasets/dbp_' + lang + '_e/exp_' + method1, num, '../datasets/dbp_' + lang + '_e/exp_'+ method +'1', rules,'../datasets/dbp_' + lang + '_e/')
    elif method == 'anchor':
        if lang == 'w':
            rules = read_rule('../datasets/D_W/exp_' + method,'../datasets/D_W')
            read_from_exp_anchor('../datasets/D_W/exp_' + method1, num, '../datasets/D_W/exp_'+ method +'1', rules,'../datasets/D_W')
        elif lang == 'y':
            rules = read_rule('../datasets/D_Y/exp_' + method,'../datasets/D_Y')
            read_from_exp_anchor('../datasets/D_Y/exp_' + method1, num, '../datasets/D_Y/exp_'+ method +'1', rules,'../datasets/D_Y')
        else:
            rules = read_rule('../datasets/dbp_' + lang + '_e/exp_' + method,'../datasets/dbp_' + lang + '_e/')
            read_from_exp_anchor('../datasets/dbp_' + lang + '_e/exp_' + method1, num, '../datasets/dbp_' + lang + '_e/exp_'+ method +'1', rules,'../datasets/dbp_' + lang + '_e/')
    else:
        if lang == 'w':
            read_from_exp('../datasets/D_W/exp_' + method, num, '../datasets/D_W/exp_'+ method +'1','../datasets/D_W')
        elif lang == 'y':
            read_from_exp('../datasets/D_Y/exp_' + method, num, '../datasets/D_Y/exp_'+ method +'1','../datasets/D_Y')
        else:
            read_from_exp('../datasets/dbp_' + lang + '_e/exp_' + method, num, '../datasets/dbp_' + lang + '_e/exp_'+ method +'1', '../datasets/dbp_' + lang + '_e/')
