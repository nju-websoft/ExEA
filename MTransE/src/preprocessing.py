...
class DBpDataset:
    def __init__(self, file_path, device, pair, lang='zh'):
        ...
        if os.path.exists(file_path + '/line_id'):
            self.line_id = self.read_line_rel(file_path + '/line_id')
        else:
            self.line_id = None
    ...
    def reconstruct(self, gid1, gid2):
        entity = set()
    ...
    def reconstruct_search(self, gid1, gid2, tri1, tri2):
        entity = set()
